...
# material type: 0->neo_hookean; 1->mooney_rivlin; 2->generalized_mooney_rivlin; 3->gent; 4->gent_gent; 5->gent_hydrogel; 6->hydrogel
materialID = 5
...
# =================================== Post-possessing =================================== #
@ti.kernel
def max_deformation() -> float:
    deform = 0.
    for i in range(particleNum[None]):
        ti.atomic_max(deform, abs(p[i].x[0] - xcenter))
    return (deform - radius) / radius
...
if __name__ == "__main__":
    ...
    while step<int(time // dt):
        ...
        for _ in range(int(0.02 * time // dt)):
            ...
            # main kernels
            solve()
            max_x = max(max_x, max_deformation())
            step+=1
    print("X/R =", max_x, ", V/Vc =", -IniVel[1]/cv)
